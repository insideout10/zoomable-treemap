...
Treemap.prototype.loadDataAndInit = function() {
  ...
  d3.json(treemapObj.config.datasetURL, function(error, json) {
    if (error) {
      ...
      treemapObj.data
        .sum(function(d) {
          if (null === d.info) return 0;
          return Math.abs(d.info.score * 1000);
        })
        .each(function(d) {
...
Treemap.prototype.initTreemap = function() {
  ...
  treemapObj.isMobilePortrait =
    DeviceDetect.mobile() && DeviceDetect.portrait();
  ...
  treemapObj.selection = d3
    .select(treemapObj.config.containerSelector)
  ...
  treemapObj.navigationHeight = parseInt(
    treemapObj.config.breadCumbs.height.replace('px', ''),
    10
  );
  treemapObj.navigation = treemapObj.selection
    .append('div')
  ...
  // Add tiles container
  treemapObj.tilesContainerHeight =
    treemapObj.config.height - treemapObj.navigationHeight;
  treemapObj.selection
    .append('div')
  ...
  treemapObj.layoutHelper = new LayoutHelper(
    layoutSize,
    treemapObj.isMobilePortrait,
    treemapObj.config
  );
...
Treemap.prototype.updateTreemap = function() {
  ...
  var tiles = treemapObj.selection
    .select('#treemap-tiles-container')
    .selectAll('.tile')
    ...
    .on('click', function(d) {
      if (d.children) {
        ...
        var element = this;
        treemapObj.animator.expandNode(
          treemapObj.selection,
          element,
          function() {
